...
(function() {
  ...
  function renderRepliesFeed() {
    ...
    if (!replies.length) {
      listEl.innerHTML = '<div class="alert alert-info"><i class="fa-solid fa-circle-info"></i> No store replies yet.</div>';
      return;
    }
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      if (!consent || !consent.checked) {
        showToast('warning', 'Consent Required', 'Please accept the terms to continue', 3000);
        document.getElementById('consent')?.parentElement?.classList.add('animate-shake');
        setTimeout(() => {
          document.getElementById('consent')?.parentElement?.classList.remove('animate-shake');
        }, 500);
        return;
      }
